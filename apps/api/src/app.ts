...
function openaiClient(): OpenAI {
  if (!process.env.OPENAI_API_KEY) {
    throw new PublicApiError("OPENAI_API_KEY is required for GPT-4o medical entity extraction.", 500);
  }
...
async function extractMedicalEntities(transcript: string, timestamp: string): Promise<ExtractionEnvelope> {
            ...
            entities: {
              ...
              items: {
                ...
                properties: {
                  ...
                  timestamp: {
                    type: "string",
                    description: "ISO-8601 timestamp. Use the supplied session timestamp unless the transcript clearly gives a different date.",
                  },
  ...
  if (!content) {
    throw new PublicApiError("GPT-4o returned no extraction content.", 502);
  }
